...
def visualization(_class_):
    ...
    data_transform, gt_transform = get_data_transforms(256, 256)
    test_path = '../../../dataset/mvtec/' + _class_
    ...
    with torch.no_grad():
        for img, gt, label, _ in test_dataloader:
            ...
            new_save_path = './results_all/' + _class_ + '/' + str(count) + '_' + 'ori.png'
            ...
            new_save_path = './results_all/'+_class_+'/'+str(count)+'_'+'ad.png'
            os.makedirs(os.path.dirname(new_save_path), exist_ok=True)
            cv2.imwrite(new_save_path, ano_map)
            # plt.imshow(ano_map)
            # plt.axis('off')
            # #plt.savefig('ad.png')
            # plt.show()
